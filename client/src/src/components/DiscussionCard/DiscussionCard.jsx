...
const DiscussionCard = ({ item }) => {
  ...
  const handleUpVote = async () => {
    try {
      ...
      const { data } = await axios.post(
        `${import.meta.env.VITE_PUBLIC_SERVER_URL}/api/v1/chat/vote/${item._id}`,
        { vote: "up" },
        config
      );
      setUp(data.upvotes);
  ...
  const handleDelete = async () => {
    try {
      ...
      const { data } = await axios.post(
        `${import.meta.env.VITE_PUBLIC_SERVER_URL}/api/v1/admin/delete-discussion`,
        { chatId: item._id },
        config
      );
      console.log(data);
  ...
  const isBookmark = async () => {
    try {
      ...
      const { data } = await axios.get(
        `${import.meta.env.VITE_PUBLIC_SERVER_URL}/api/v1/users/${user.data.user.name}`,
  ...
  const doBookmark = async () => {
    try {
      ...
      const { data } = await axios.post(
        `${import.meta.env.VITE_PUBLIC_SERVER_URL}/api/v1/users/add-bookmark`,
        { chatId: item._id },
        config
      );
  ...
  return (
    <div className="discussion-card">
      {/* {console.log(item)} */}
      {report ? <ReportPopup setReport={setReport} item={item._id ? item._id : ""} /> : ""}
      ...
      <div className="discussion-card-datas">
        <div className="discussion-card-data">
          ...
          <div className="discussion-card-comment">
            ...
            >
              <BiComment color="#000" className="discussion-icon" />
              {/* <p>4</p> */}
            </Link>
            {item
              ? item.users.map((user) => (
