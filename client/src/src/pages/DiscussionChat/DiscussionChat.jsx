...
const DiscussionChat = () => {
  ...
  const handleUpVote = async () => {
    try {
      ...
      const { data } = await axios.post(
        `${import.meta.env.VITE_PUBLIC_SERVER_URL}/api/v1/chat/vote/${discussionData._id}`,
        { vote: "up" },
        config
      );
      setUp(data.upvotes);
  ...
  const pageLoad = async () => {
    try {
      ...
      const { data } = await axios.post(
        `${import.meta.env.VITE_PUBLIC_SERVER_URL}/api/v1/chat/slug`,
        { slug: slug },
        config
      );
      ...
      const message = await axios.get(
        `${import.meta.env.VITE_PUBLIC_SERVER_URL}/api/v1/message/${data.chat[0]._id}`,
